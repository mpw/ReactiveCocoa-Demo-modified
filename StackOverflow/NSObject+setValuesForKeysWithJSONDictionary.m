    ...
    for (Class clas = [self class]; clas; clas = class_getSuperclass(clas)) {
        ...
        for (int i=0; i<propertyCount; i++) {
            ...
            if (value == nil) {
                ...
                if (ivarPropertyName != NULL) {  // Not a totally alien property?
                    NSString *ivarName = @(ivarPropertyName);
                    value = keyedValues[ivarName];
...
- (void)bnrSetValuesForKeysWithJSONDictionary:(NSDictionary *)keyedValues {
    [self bnrSetValuesForKeysWithJSONDictionary:keyedValues dateFormatter:nil];
}
